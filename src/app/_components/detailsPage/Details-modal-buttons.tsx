...
const DetailsModalButtons = ({loading}: Buttons) => {
  ...
  return (
    <div>
            <div className='text-sm font-medium'>
                {loading ? (
                    <div className='flex items-center justify-center h-10 w-full rounded-md text-white bg-loading'>
            ...
            transition duration-300 flex justify-center items-center ${loading ? 'pointer-events-none' : ''}`}>Рассчитать стоимость</Link>
            <button className={`mt-2 rounded-md w-full h-10 bg-btn text-white hover:bg-lime-500 transition duration-300 ${loading ? 'pointer-events-none' : ''}`}
            onClick={clickHandle}>Оставить заявку</button>
            { modalOpened ? (
